...
def example_vega_analysis():
    ...
    for strike in strikes:
        ...
        def get_vega_value(vega_report, strike, i, j):
            ...
            key = f'VEGA:{strike}:{i}_{j}:ZC:VEGAVALUE:NA'
...
def example_full_sensitivity_report():
    ...
    all_sensitivities = analyzer.compute_all_sensitivities(
        compute_delta=False,#True,
        compute_vega=False,#True,
        compute_gamma=False,  # Skip gamma for brevity
    ...
    # folder=r"E:\OneDrive\Dropbox\LinearRationalWishart_Work\Code\ED\LinearRationalWishart\LinearRationalWishart_NewCode\wishart_processes\examples\results"
...
def example_hedging_analysis():
    ...
    for mult in strike_multipliers:
        ...
        hedging_results.append({
            ...
            'Moneyness': mult,
            'Price': price,
            'ZC_Hedge_Count': len([k for k in zc_hedge.keys() if 'amount' in k]),
            'Swap_Fixed_Hedge': swap_hedge.get('Swap_Hedge_Fixed_amount', 0),
            'Swap_Float_Hedge': swap_hedge.get('Swap_Hedge_Floating_amount', 0)
        })
